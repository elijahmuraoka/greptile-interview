...
import { ChevronLeft, ChevronRight, Rocket, PartyPopper, Undo2 } from 'lucide-react';
...
export default function GenerateChangelogModal({
  ...
  const handleGenerateChangelog = async () => {
    ...
    try {
      ...
      toast({
        description: (error as Error).message || 'Failed to generate changelog.',
        variant: 'destructive',
      });
    } finally {
            ...
            >
              ...
              <div className="space-y-4">
                <Button className="w-full bg-green-500 hover:bg-green-600 transition-colors">
                  <Link href={`/directory/${changelog?.id}`}>View Changelog</Link>
                </Button>
                <Button
                  variant="outline"
      ...
      <DialogContent className="min-h-[98vh] max-w-[98vw] flex flex-col justify-between gap-6 items-center p-6">
        ...
        <div className="flex justify-between w-full">
          ...
          {currentStep < steps.length - 1 && (
            <Button
              onClick={handleNext}
              disabled={(currentStep === 0 && !changelog) || (currentStep === 1 && !changelog)}
            >
              Next
